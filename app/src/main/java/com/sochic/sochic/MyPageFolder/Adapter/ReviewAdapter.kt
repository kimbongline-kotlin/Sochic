...
) : androidx.recyclerview.widget.RecyclerView.Adapter<ReviewAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(parent: ViewGroup) : androidx.recyclerview.widget.RecyclerView.ViewHolder(
        ...
        fun onBindViewHolder( item : ReviewAPI.ReviewList) {
            with(itemView) {
                ...
                if(!item.review_confirm) {
                    rWriteBtn.visibility = View.VISIBLE
                    ratingbar.visibility = View.GONE

                    rWriteBtn.setOnClickListener {
                        mAct.startActivity(Intent(mCon,ReviewWriteActivity::class.java).setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP)
                            .putExtra("idx",item.sub_o_index)
                            .putExtra("ocode",item.o_code))
                    }
                }else {
