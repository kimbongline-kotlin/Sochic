...
import com.sochic.sochic.Util.Utility;
...
public class PayWebActivity extends ScActivity {
    ...
    final String MERCHANT_URL = "http://sochic.app-pick.com/pay_module/MOBILE_TX_PHP/payRequest_utf.php";	//가맹점의 결제 요청 페이지 URL
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        postOcode = intent.getStringExtra("o_code");

        Uri uri = intent.getData();
        mWebView = (WebView) findViewById(R.id.webView);
        mWebView.setWebChromeClient(new WebChromeClient());
        mWebView.addJavascriptInterface(new AndroidBridge(), "Payment");
        ...
        */
        ...
        if( uri != null ) {
             ...
             */
            String postData = "o_code=" + postOcode;
            Log.d("object",postData);
            // 결제 요청할 가맹점의 웹페이지를 호출합니다.
            // 가맹점에 맞는 URL로 변경하십시요. 해당 웹페이지는 NICEPAY_SMART_WEB을 참조합니다.
            mWebView.postUrl(MERCHANT_URL, EncodingUtils.getBytes(postData,"UTF-8"));
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if("091".equals(resCode)){//계좌이체 결제를 취소한 경우
            Utility.AlertDialog("인증 오류", "계좌이체 결제를 취소하였습니다.",  PayWebActivity.this);
            mWebView.postUrl(MERCHANT_URL, null);
        } else if("060".equals(resCode)){
            Utility.AlertDialog("인증 오류", "타임아웃",  PayWebActivity.this);
            mWebView.postUrl(MERCHANT_URL, null);
        } else if("050".equals(resCode)){
            Utility.AlertDialog("인증 오류", "전자서명 실패",  PayWebActivity.this);
            mWebView.postUrl(MERCHANT_URL, null);
        } else if("040".equals(resCode)){
            Utility.AlertDialog("인증 오류", "OTP/보안카드 처리 실패",  PayWebActivity.this);
            mWebView.postUrl(MERCHANT_URL, null);
        } else if("030".equals(resCode)){
            Utility.AlertDialog("인증 오류", "인증모듈 초기화 오류",  PayWebActivity.this);
            mWebView.postUrl(MERCHANT_URL, null);
    ...
    private class AndroidBridge {
        ...
        @JavascriptInterface
        public void setResult(final boolean str) {

            if(str) {
                finish();
                ShowToast("결제가 완료되었습니다.");
            }
            Log.d("object",String.valueOf(str));
        }
    }
}
